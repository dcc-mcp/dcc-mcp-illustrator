...
def main() -> None:
    ...
    required_wheel = {
        ...
        "dcc_mcp_illustrator/skills/illustrator-artwork/SKILL.md",
        "dcc_mcp_illustrator/skills/illustrator-document/SKILL.md",
        "dcc_mcp_illustrator/skills/illustrator-export/SKILL.md",
    }
    ...
    with zipfile.ZipFile(wheels[0]) as archive:
        ...
        skill_contents = {
            skill: archive.read(f"dcc_mcp_illustrator/skills/{skill}/SKILL.md").decode("utf-8")
            for skill in (
                "illustrator-artwork",
    ...
    for skill, content in skill_contents.items():
        if "dcc-mcp-core >=0.20.14,<1.0.0" not in content or "dcc-mcp-core 0.19+" in content:
            raise SystemExit(f"wheel skill declares an unsupported Core range: {skill}")
